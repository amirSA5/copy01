...
function ParametresProfiler () {
  ...
  const ajoutsousArticle = e => {
    axios
      .post ('http://localhost:4000/app/Ajout_sousArticle', {
        Nom: ajoutsousArticleNom.Nom,
        article: articleID,
      })
  ...
  const ajoutAttribut = e => {
    axios
      .post ('http://localhost:4000/app/Ajout_Attribut', {
        Nom: ajoutAttributNom.Nom,
        sousArticle: sousArticleID,
      })
      .then (response => console.log (response.data));
    console.log (articleID);
    setOpenAttribut (false);
  ...
  const ajoutProfiler = e => {
    axios
      .post ('http://localhost:4000/app/Ajout_profiler', {
        reference: profilerReference.reference,
        formule: profilerFormule.formule,
        attribut: attributID,
      })
      .then (response => console.log (response.data));
    e.preventDefault ();
    ...
    <div className="parametres">
      <Link to="/">
        <Button size="large" variant="contained" color="success">
          <KeyboardDoubleArrowLeftIcon />RETOUR
        </Button>
      </Link>
          ...
          <tr className="tr_parametres_Table">
            {article.map (data => {
              return (
                <td key={data._id}>
                  <Button
                    onClick={() => afficherSousArticle (data.Nom, data._id)}
          ...
          <tr className="tr_parametres_Table">
            {sousArticle.map (data => {
              return (
                <td key={data._id}>
                  <Button
                    onClick={() => afficherProfiler (data.Nom, data._id)}
                    className="paramétres_profiler"
                    size="large"
                    variant="contained"
                    color="success"
                  ...
                  <Button
                    onClick={() => deleteSousArticle (data._id)}
                    className="paramétres_profiler"
                    size="large"
                    variant="contained"
                    color="error"
            ...
            {attribut.map (data => {
              return (
                <tr key={data._id}>
                  <td>
                    <Button
                      onClick={() => deleteAttribut (data._id)}
                      className="paramétres_profiler"
                      size="small"
                      variant="contained"
                      color="error"
                    ...
                    <Button
                      onClick={() => handleOpenProfiler (data._id)}
                      className="paramétres_profiler"
                      size="small"
                      variant="contained"
                      color="secondary"
                    >
                      <AddCircleOutlinedIcon fontSize="meduim" />
                    </Button>
                  </td>
